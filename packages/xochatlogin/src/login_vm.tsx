...
export class LoginVM extends ProviderListener {
    ...
    async requestLogin(authCode: string) {
        ...
        const resp = await XOApp.apiClient.post(`user/login_authcode/${authCode}`);
        if (resp) {
    ...
    async requestLoginWithUsernameAndPwd(username: string, password: string) {
        this.loginLoading = true
        this.notifyListener()
        return XOApp.apiClient.post(`user/login`, { "username": username, "password": password, "flag": 1 }).then((result)=>{
    ...
    loginSuccess(data:any) {
        const loginInfo = XOApp.loginInfo
    ...
    requestUUID() {
        ...
        this.notifyListener()
        XOApp.apiClient.get('user/loginuuid').then((result) => {
    ...
    pullLoginStatus(uuid?: string) {
        ...
        XOApp.apiClient.get(`user/loginstatus?uuid=${uuid}`).then((result: any) => {
            this._pullErrCount = 0
