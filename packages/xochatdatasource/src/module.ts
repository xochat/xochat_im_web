import { GroupRole, IModule, XOApp } from "@xochat/base"
import { Channel, ChannelInfo, ChannelTypeGroup, ChannelTypePerson, Conversation, XOSDK, Message, Subscriber, ConversationExtra, Reminder } from "xochat_js_sdk";
import { MessageTask } from "xochat_js_sdk";
...
export default class DataSourceModule implements IModule {
    ...
    init(): void {
        ...
        XOApp.conversationProvider = new ConversationProvider()

        XOApp.dataSource.channelDataSource = new ChannelDataSource()
        XOApp.dataSource.commonDataSource = new CommonDataSource()
    ...
    setChannelInfoCallback() {
        XOSDK.shared().config.provider.channelInfoCallback = async function (channel: Channel): Promise<ChannelInfo> {
            let channelInfo = new ChannelInfo(),
                isUsers = channel.channelType === ChannelTypePerson;
            const resp = await XOApp.apiClient.get(`channels/${channel.channelID}/${channel.channelType}`);
    ...
    setSyncSubscribersCallback() {
        XOSDK.shared().config.provider.syncSubscribersCallback = async function (channel: Channel, version: number): Promise<Array<Subscriber>> {
            const resp = await XOApp.apiClient.get(`groups/${channel.channelID}/membersync?version=${version}&limit=10000`);
            let members = [];
            if (resp) {
                for (let i = 0; i < resp.length; i++) {
                    ...
                    member.avatar = XOApp.shared.avatarUser(member.uid)
                    members.push(member);
                }
            }
            members.sort((a, b) => {
    ...
    setMessageUploadTaskCallback() {
        // 消息上传任务
        XOSDK.shared().config.provider.messageUploadTaskCallback = (message: Message): MessageTask => {
            return new MediaMessageUploadTask(message)
        }
    }

    setSyncConversationExtrasCallback() {
        XOSDK.shared().config.provider.syncConversationExtrasCallback = async (version: number) => {
            let conversationExtras = new Array<ConversationExtra>();
            const results = await XOApp.apiClient.post("conversation/extra/sync", { "version": version })
            if (results) {
    ...
    setSyncMessageExtraCallback() {
        XOSDK.shared().config.provider.syncMessageExtraCallback = async (channel: Channel, extraVersion: number, limit: number) => {
            return XOApp.conversationProvider.syncMessageExtras(channel, extraVersion, limit)
        }
    }

    setSyncRemindersCallback() {
        XOSDK.shared().config.provider.syncRemindersCallback = async (version: number) => {
            let reminders = new Array<Reminder>();
            const channelIDs = new Array<string>()
            const conversations = XOSDK.shared().conversationManager.conversations
            ...
            const results = await XOApp.apiClient.post("message/reminder/sync", { "version": version, "limit": 100, "channel_ids": channelIDs })
            if (results) {
                for (const result of results) {
    ...
    setReminderDoneCallback() {
        XOSDK.shared().config.provider.reminderDoneCallback = async (ids: number[]) => {
            return XOApp.apiClient.post("message/reminder/done", ids)
        }
    }

    setMessageReadedCallback() {
        XOSDK.shared().config.provider.messageReadedCallback = async (channel: Channel, messages: Message[]) => {
            const messageIDs = []
            ...
            return XOApp.apiClient.post("message/readed", { "channel_id": channel.channelID, "channel_type": channel.channelType, "message_ids": messageIDs }).catch((err) => {
    ...
    setSyncConversationsCallback() {
        XOSDK.shared().config.provider.syncConversationsCallback = async (filter?: any): Promise<Array<Conversation>> => {
            let resp: any
            let conversations = new Array<Conversation>();
            resp = await XOApp.apiClient.post("conversation/sync", { "msg_count": 1 })
            if (resp) {
                ...
                if (users && users.length > 0) {
                    for (const user of users) {
                        XOSDK.shared().channelManager.setChannleInfoForCache(Convert.userToChannelInfo(user))
                    }
                }
                const groups = resp.groups
                if (groups && groups.length > 0) {
                    for (const group of groups) {
                        XOSDK.shared().channelManager.setChannleInfoForCache(Convert.groupToChannelInfo(group))
                    }
                }
            }
