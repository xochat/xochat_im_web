...
import { XOSDK, ChannelInfo, Channel, Conversation, Message, MessageStatus, ChannelTypePerson, ChannelTypeGroup,ConversationExtra,Reminder, MessageExtra } from "xochat_js_sdk";
...
export class Convert {
    ...
    static toMessage(msgMap: any): Message {
        ...
        const messageContent = XOSDK.shared().getMessageContent(contentType)
        if (contentObj) {
    ...
    static toMessageExtra(msgExtraMap: any) :MessageExtra {
        ...
        if(contentEditObj) {
            ...
            const contentEditContent = XOSDK.shared().getMessageContent(contentEditContentType)
