import { ChannelQrcodeResp, Contacts, IChannelDataSource, ICommonDataSource, XOApp, RequestConfig } from "@xochat/base";
import { Channel, ChannelInfo, ChannelTypeGroup, ChannelTypePerson, XOSDK, Message, MessageContentType,ConversationExtra } from "xochat_js_sdk";
...
export class ChannelDataSource implements IChannelDataSource {
    ...
    async exitChannel(channel: Channel): Promise<void> {
        ...
        return XOApp.apiClient.post(`groups/${channel.channelID}/exit`)
    }
    ...
    async channelTransferOwner(channel: Channel, toUID: string): Promise<void> {
        ...
        return XOApp.apiClient.post(`groups/${channel.channelID}/transfer/${toUID}`)
    }
    ...
    async subscriberAttrUpdate(channel: Channel, subscriberUID: string, attr: any): Promise<any> {
        ...
        return XOApp.apiClient.put(`groups/${channel.channelID}/members/${subscriberUID}`, attr)
    }
    createChannel(uids: string[]): Promise<any> {
        return XOApp.apiClient.post(`group/create`, { members: uids });
    }
    async groupSaveList(): Promise<ChannelInfo[]> {
        const resp = await XOApp.apiClient.get('group/my', {
            param: {
                "limit": 100000,
            }
        });
        ...
        if (resp) {
            ...
            for (const data of resp) {
                ...
                channelInfo.logo = XOApp.shared.avatarChannel(channelInfo.channel);
    ...
    removeSubscribers(channel: Channel, uids: string[]): Promise<void> {
        return XOApp.apiClient.delete(`groups/${channel.channelID}/members`, {
            data: {
                members: uids,
            }
        })
    ...
    addSubscribers(channel: Channel, uids: string[]): Promise<void> {
        return XOApp.apiClient.post(`groups/${channel.channelID}/members`, {
            members: uids,
        })
    }
    ...
    updateField(channel: Channel, field: string, value: string): Promise<void> {
        const param: any = {}
        param[field] = value
        return XOApp.apiClient.put(`groups/${channel.channelID}`, param)
    }

    qrcode(channel: Channel): Promise<ChannelQrcodeResp> {
        return XOApp.apiClient.get(`groups/${channel.channelID}/qrcode`, {
            resp: () => {
    ...
    async updateSetting(setting: any, channel: Channel): Promise<void> {
        if (channel.channelType === ChannelTypeGroup) {
            return XOApp.apiClient.put(`groups/${channel.channelID}/setting`, setting)
        } else if (channel.channelType === ChannelTypePerson) { // 个人信息
            return XOApp.apiClient.put(`users/${channel.channelID}/setting`, setting)
        }
    }

    async managerRemove(channel: Channel, uids: string[]): Promise<void> {
        return XOApp.apiClient.delete(`groups/${channel.channelID}/managers`, {
            data: uids,
        })
    }

    async managerAdd(channel: Channel, uids: string[]): Promise<void> {
        return XOApp.apiClient.post(`groups/${channel.channelID}/managers`, uids)
    }

    blacklistAdd(channel: Channel, uids: string[]): Promise<void> {
        return XOApp.apiClient.post(`groups/${channel.channelID}/blacklist/add`, { uids: uids })
    }


    blacklistRemove(channel: Channel, uids: string[]): Promise<void> {
        return XOApp.apiClient.post(`groups/${channel.channelID}/blacklist/remove`, { uids: uids })
    }

    conversationExtraUpdate(conversationExtra:ConversationExtra): Promise<void> {
        return XOApp.apiClient.post(`conversations/${conversationExtra.channel.channelID}/${conversationExtra.channel.channelType}/extra`,{
...
export class CommonDataSource implements ICommonDataSource {
    blacklistAdd(uid: string): Promise<void> {
        return XOApp.apiClient.post(`user/blacklist/${uid}`)
    }
    blacklistRemove(uid: string): Promise<void> {
        return XOApp.apiClient.delete(`user/blacklist/${uid}`)
    }
    deleteFriend(uid:string): Promise<void> {
        return XOApp.apiClient.delete(`friends/${uid}`)
    }

    userRemark(uid: string, remark: string): Promise<void> {
        return XOApp.apiClient.put(`friend/remark`, { uid: uid, remark: remark })
    }
    getFavoritesAll(): Promise<any> {
        // TODO: 这里先取10000足够 等后面再做分页
        return XOApp.apiClient.get(`favorite/my?page_index=1&page_size=10000`)
    }
    favorities(message: Message): Promise<void>{
        ...
        return XOApp.apiClient.post(`favorites`, {
    ...
    favoritiesDelete(id: string): Promise<void> {
        return XOApp.apiClient.delete(`favorites/${id}`)
    }
    userStickerCategory(): Promise<any> {
        return XOApp.apiClient.get(`sticker/user/category`)
    }
    getStickers(category: string): Promise<any> {
        return XOApp.apiClient.get(`sticker/user/sticker?category=${category}`)
    }
    searchUser(keyword: string): Promise<any> {
        return XOApp.apiClient.get(`user/search?keyword=${keyword}`)
    }
    qrcodeMy(): Promise<any> {
        return XOApp.apiClient.get("user/qrcode")
    }

    friendSure(token: string): Promise<void> {
        return XOApp.apiClient.post("friend/sure", {
            "token": token,
        })
    }

    friendApply(req:{uid:string,remark:string,vercode:string}):Promise<void> {
        return XOApp.apiClient.post(`friend/apply`,{to_uid:req.uid,remark:req.remark,vercode:req.vercode},)
    }
    ...
    */
    ...
    async contactsSync(version: string): Promise<Contacts[]> {
        const results = await XOApp.apiClient.get(`friend/sync`, {
    ...
    imConnectAddr(): Promise<string> {
        return XOApp.apiClient.get(`users/${XOApp.loginInfo.uid}/im`).then((resp) => {
    ...
    toContacts(resultDic: any): Contacts {
        ...
        contacts.avatar = XOApp.shared.avatarUser(contacts.uid)
    ...
    async searchFriends(keyword?: string): Promise<ChannelInfo[]> {
        let resp = await XOApp.apiClient.get('friend/sync', {
            param: {
                "keyword": keyword,
                "api_version":"1"
            }
