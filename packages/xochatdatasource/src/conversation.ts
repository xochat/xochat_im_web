import { IConversationProvider, XOApp, SyncMessageOptions } from "@xochat/base";
...
export class ConversationProvider implements IConversationProvider {
    ...
    revokeMessage(message: Message): Promise<void> {
        return XOApp.apiClient.post(`message/revoke?channel_id=${message.channel.channelID}&channel_type=${message.channel.channelType}&message_id=${message.messageID}&client_msg_no=${message.clientMsgNo}`)
    }


    markConversationUnread(channel: Channel, unread: number): Promise<void> {
        return XOApp.apiClient.put('coversation/clearUnread', { "channel_id": channel.channelID, "channel_type": channel.channelType, "unread": unread > 0 ? unread : 0 }).catch(function (error) {
            console.log(error);
            return error;
        });
    ...
    async syncGetMessages(channel: Channel, opts: SyncMessageOptions): Promise<Message[]> {
        ...
        var resp = await XOApp.apiClient.post(`message/channel/sync`, { "limit": limit, "channel_id": channel.channelID, "channel_type": channel.channelType, "start_message_seq": opts.startMessageSeq || 0, "end_message_seq": opts.endMessageSeq || 0, "pull_mode": opts.pullMode });
    ...
    async syncMessageExtras(channel: Channel, version: number, limit: number): Promise<MessageExtra[]> {
        ...
        let messageExtraObjs = await XOApp.apiClient.post(`message/extra/sync`, { "channel_id": channel.channelID, "channel_type": channel.channelType, "extra_version": version, "limit": limit })
        if(messageExtraObjs) {
