...
import { Channel, ChannelInfo, ChannelTypePerson } from "xochat_js_sdk";
...
export default class ConversationList extends Component<ConversationListProps, ConversationListState>{
    ...
    lastContent(conversationWrap: ConversationWrap) {
        ...
        if (lastMessage.channel.channelType === ChannelTypePerson) {
            ...
            if (lastMessage.fromUID && lastMessage.fromUID !== "") {
                ...
                const fromChannelInfo = XOSDK.shared().channelManager.getChannelInfo(fromChannel)
                if (fromChannelInfo) {
    ...
    conversationItem(conversationWrap: ConversationWrap) {
        ...
        if (!channelInfo) {
            XOSDK.shared().channelManager.fetchChannelInfo(conversationWrap.channel)
        }
    ...
    onCloseChat(channel: Channel) { // 关闭聊天
        XOApp.conversationProvider.deleteConversation(channel)
    }

    async onClearMessages(channel: Channel) {
        const conversation = XOSDK.shared().conversationManager.findConversation(channel)
        if (!conversation) {
            return
        }
        await XOApp.conversationProvider.clearConversationMessages(conversation)
        conversation.lastMessage = undefined
        XOApp.endpointManager.invoke(EndpointID.clearChannelMessages, channel)
        this.setState({})
    }
