import { Channel, ChannelTypeGroup, ChannelTypePerson, ConversationAction, XOSDK, Message, MessageContent, MessageStatus, Subscriber, Conversation, MessageExtra,CMDContent, PullMode } from "xochat_js_sdk";
...
export default class ConversationVM extends ProviderListener {
    ...
    markUnread() {
        if (this.needSetUnread) {
            XOApp.conversationProvider.markConversationUnread(this.channel, this.unreadCount)
        }
    }
    ...
    sendMergeforward(toChannels: Channel[]) {
        ...
        if (checkedMessages && checkedMessages.length > 0) {
            for (const message of checkedMessages) {
                let channelInfo = XOSDK.shared().channelManager.getChannelInfo(new Channel(message.fromUID, ChannelTypePerson))
    ...
    async deleteMessages(deletedMessages: Message[]): Promise<void> {
        ...
        XOApp.conversationProvider.deleteMessages(deletedMessages)
    ...
    async revokeMessage(message: Message): Promise<void> {
        ...
        return XOApp.conversationProvider.revokeMessage(message)
    ...
    async deleteMessagesFromLocal(deletedMessages: Message[]): Promise<void> {
        ...
        for (const deletedMessage of deletedMessages) {
            XOApp.shared.notifyMessageDeleteListener(deletedMessage, lastMessage)
        }
    ...
    didMount(): void {
        ...
        XOSDK.shared().chatManager.addMessageListener(this.messageListener)
        ...
        this.cmdListener = (message: Message) => {
            ...
            if (cmdContent.cmd === 'messageRevoke') { //消息撤回
                ...
                if (message.channel.isEqual(this.channel)) {
                    XOSDK.shared().chatManager.syncMessageExtras(this.channel, this.findMaxExtraVersion()).then((messageExtras)=>{
        ...
        XOApp.endpointManager.setMethod(EndpointID.clearChannelMessages, (channel: Channel) => {
        ...
        if (this.channel.channelType === ChannelTypeGroup) {
            ...
            XOSDK.shared().channelManager.addSubscriberChangeListener((channel: Channel) => {
                if (!this.channel.isEqual(channel)) {
                    return
                }
                this.reloadSubscribers()
            })
            XOSDK.shared().channelManager.syncSubscribes(this.channel)
        ...
        const conversation = XOSDK.shared().conversationManager.findConversation(this.channel)
        if (conversation) {
            ...
            XOSDK.shared().conversationManager.openConversation = conversation
        }
    ...
    didUnMount(): void {
        ...
        XOSDK.shared().chatManager.removeMessageListener(this.messageListener)
        XOSDK.shared().chatManager.removeMessageStatusListener(this.messageStatusListener)
        XOApp.endpointManager.removeMethod(EndpointID.clearChannelMessages)
        XOSDK.shared().chatManager.removeCMDListener(this.cmdListener)
    ...
    markReminderDones() {
        ...
        if (conversation && conversation.reminders && conversation.reminders.length > 0) {
            ...
            if (ids.length > 0) {
                XOSDK.shared().reminderManager.done(ids)
            }
        }
    ...
    // 重新加载订阅者
    reloadSubscribers() {
        this.subscribers = XOSDK.shared().channelManager.getSubscribes(this.channel)
        this.notifyListener()
    }
    ...
    appendMessage(messageWrap: MessageWrap) {
        ...
        const senderIsSelf = messageWrap.fromUID === XOApp.loginInfo.uid
    ...
    refreshNewMsgCount() {
        ...
        if (oldUnreadCount != this.unreadCount) {
            ...
            if (conversation) {
                ...
                XOSDK.shared().conversationManager.notifyConversationListeners(conversation, ConversationAction.update)
            }
        }
    ...
    async syncMessages(initMessageSeq?: number, stateCallback?: () => void) {
        ...
        opts.limit = XOApp.config.pageSizeOfMessage
        ...
        const remoteMessages = await XOApp.conversationProvider.syncMessages(this.channel, opts)
    ...
    async pulldownMessages() {
        ...
        let remoteMessages = await XOApp.conversationProvider.syncMessages(this.channel, opts)
        const newMessages = new Array<Message>()
        if(remoteMessages && remoteMessages.length>0) {
            remoteMessages.forEach(msg => {
                 if (!msg.isDeleted) {
    ...
    async sendMessage(content: MessageContent, channel: Channel): Promise<Message> {
        ...
        const message = await XOSDK.shared().chatManager.send(content, channel, setting)
        const messageWrap = new MessageWrap(message)
