...
import XOApp, { FriendApply, FriendApplyState, ThemeMode } from "./App";
...
export default class BaseModule implements IModule {
  ...
  init(): void {
    ...
    XOApp.messageManager.registerMessageFactor(
    ...
    XOSDK.shared().register(MessageContentType.image, () => new ImageContent()); // 图片

    XOSDK.shared().register(MessageContentTypeConst.card, () => new Card()); // 名片
    ...
    // 未知消息
    XOApp.messageManager.registerCell(
    ...
    XOSDK.shared().chatManager.addCMDListener((message: Message) => {
      ...
      if (cmdContent.cmd === "channelUpdate") {
        // 频道信息更新
        XOSDK.shared().channelManager.fetchChannelInfo(
        ...
        const conversation =
          XOSDK.shared().conversationManager.findConversation(channel);
        let unread = 0;
        if (param.unread > 0) {
          unread = param.unread;
        }
        if (conversation) {
          ...
          XOSDK.shared().conversationManager.notifyConversationListeners(
            conversation,
        ...
        // 最近会话删除
        const channel = new Channel(param.channel_id, param.channel_type);
        XOSDK.shared().conversationManager.removeConversation(channel);
        ...
        XOApp.shared.addFriendApply(friendApply);
        ...
        XOApp.dataSource.contactsSync(); // 同步联系人

        const friendApplys = XOApp.shared.getFriendApplys();
        if (friendApplys && friendApplys.length > 0) {
          for (const friendApply of friendApplys) {
            if (toUID === friendApply.uid) {
              ...
              XOApp.shared.updateFriendApply(friendApply);
              XOApp.endpointManager.invokes(
        ...
        XOSDK.shared().channelManager.syncSubscribes(channel);
    ...
    XOSDK.shared().chatManager.addMessageListener((message: Message) => {
      ...
      switch (message.contentType) {
        case MessageContentTypeConst.channelUpdate:
          XOSDK.shared().channelManager.fetchChannelInfo(message.channel);
          break;
        case MessageContentTypeConst.addMembers:
        case MessageContentTypeConst.removeMembers:
          XOSDK.shared().channelManager.syncSubscribes(message.channel);
          break;
      }
      ...
      if (this.allowNotify(message)) {
        ...
        if (message.channel.channelType === ChannelTypeGroup) {
          const fromChannelInfo = XOSDK.shared().channelManager.getChannelInfo(
    ...
    XOSDK.shared().channelManager.addListener((channelInfo: ChannelInfo) => {
      if (channelInfo.channel.channelType === ChannelTypePerson) {
        if (XOApp.loginInfo.uid === channelInfo.channel.channelID) {
  ...
  allowNotify(message: Message) {
    if (XOApp.shared.notificationIsClose) {
      // 用户关闭了通知
      return false;
    }
    if (XOSDK.shared().isSystemMessage(message.contentType)) {
      // 系统消息不发通知
      return false;
    }
    if (message.fromUID === XOApp.loginInfo.uid) {
      // 自己发的消息不发通知
      return false;
    }
  ...
  sendNotification(message: Message, description?: string) {
    let channelInfo = XOSDK.shared().channelManager.getChannelInfo(
      message.channel
    );
    ...
    if (window.Notification && Notification.permission !== "denied") {
      ...
      notify.onclick = () => {
        notify.close();
        window.focus();
        XOApp.endpoints.showConversation(message.channel);
      };
      notify.onshow = () => {
        //5秒后关闭消息框
        setTimeout(function () {
  ...
  registerChatMenus() {
    XOApp.shared.chatMenusRegister("chatmenus.startchat", (param) => {
      const isDark = XOApp.config.themeMode === ThemeMode.dark;
      return {
        title: "发起群聊",
        icon: require(`${
            ...
            <ContactsSelect
              ...
              onFinished={() => {
                if (selectItems && selectItems.length > 0) {
                  ...
                  XOApp.dataSource.channelDataSource
                    .createChannel(
  ...
  registerMessageContextMenus() {
    ...
    XOApp.endpoints.registerMessageContextMenus(
      ...
      (message, context) => {
        if (XOApp.shared.notSupportForward.includes(message.contentType)) {
          return null;
        }
            ...
            properties: {
              ...
              onClick: () => {
                this.inputEditPush(
                  ...
                  async (value) => {
                    await XOApp.dataSource.commonDataSource
        ...
        if (fromSubscriberOfUser) {
          ...
          if (
            ...
            const inviteChannelInfo =
              XOSDK.shared().channelManager.getChannelInfo(inviterChannel);
            if (inviteChannelInfo) {
              joinDesc += ` ${inviteChannelInfo.title}邀请入群`;
            } else {
              XOSDK.shared().channelManager.fetchChannelInfo(inviterChannel);
            }
          } else {
            joinDesc += "加入群聊";
          }
          ...
          rows.push(
            new Row({
              ...
              properties: {
                title: "解除好友关系",
                onClick: () => {
                  XOApp.shared.baseContext.showAlert({
                    ...
                    onOk: () => {
                      XOApp.dataSource.commonDataSource
                        ...
                        .then(() => {
                          ...
                          const conversation =
                            XOSDK.shared().conversationManager.findConversation(
                              channel
                            );
                          if (conversation) {
                            XOApp.conversationProvider.clearConversationMessages(
                              conversation
                            );
                          }
                          XOSDK.shared().conversationManager.removeConversation(
                            channel
                          );
                          XOApp.endpointManager.invoke(
  ...
  registerChannelSettings() {
    XOApp.shared.channelSettingRegister("channel.subscribers", (context) => {
                  ...
                  ></ContactsSelect>,
                  {
                    ...
                    onFinish: async () => {
                      ...
                      if (channel.channelType === ChannelTypePerson) {
                        ...
                        uids.push(XOApp.loginInfo.uid || "");
                        uids.push(channel.channelID);
                        for (const item of addSelectItems) {
                          uids.push(item.id);
                        }

                        const result = await XOApp.dataSource.channelDataSource
                          .createChannel(uids)
                          .catch((err) => {
                            Toast.error(err.msg);
                          });
                        if (result) {
                          XOApp.endpoints.showConversation(
                            new Channel(result.group_no, ChannelTypeGroup)
                          );
                        }
                      } else {
                        await XOApp.dataSource.channelDataSource.addSubscribers(
                          channel,
                ...
                context.push(
                  <UserSelect
                    ...
                    users={data.subscribers
                      .filter(
                        (subscriber) => subscriber.uid !== XOApp.loginInfo.uid
                      )
                      .map((item) => {
                  ...
                  ></UserSelect>,
                  {
                    ...
                    onFinish: async () => {
                      ...
                      await XOApp.dataSource.channelDataSource.removeSubscribers(
                        channel,
            ...
            properties: {
              ...
              onClick: () => {
                ...
                this.inputEditPush(
                  ...
                  channelInfo?.title || "",
                  (value: string) => {
                    return XOApp.dataSource.channelDataSource
            ...
            properties: {
              ...
              icon: (
                <img
                  ...
                  src={XOApp.shared.avatarChannel(channel)}
                ></img>
              ),
              onClick: () => {
                context.push(
              ...
              properties: {
                ...
                onClick: () => {
                  this.inputEditPush(
                    ...
                    name || "",
                    (value: string) => {
                      return XOApp.dataSource.channelDataSource.subscriberAttrUpdate(
                        data.channel,
                        XOApp.loginInfo.uid || "",
                        { remark: value }
                      );
    ...
    // XOApp.shared.channelSettingRegister("channel.notify.setting.screen", (context) => {
    //     return new Section({
    ...
    //     })
    // })
    // XOApp.shared.channelSettingRegister("channel.notify.setting.revokemind", (context) => {
    //     return new Section({
    ...
    //     })
    // })
    // XOApp.shared.channelSettingRegister("channel.base.setting5", (context) => {
    //     return new Section({
    //         rows: [
    //             new Row({
    //                 cell: ListItem,
                ...
                onClick: () => {
                  XOApp.shared.baseContext.showAlert({
                    ...
                    onOk: async () => {
                      ...
                      await XOApp.conversationProvider.clearConversationMessages(
                        conversation
                      );
                ...
                onClick: () => {
                  XOApp.shared.baseContext.showAlert({
                    ...
                    onOk: async () => {
                      ...
                      XOApp.conversationProvider.deleteConversation(
                        data.channel
                      );
                    },
                  });
                    ...
                    <UserSelect
                      ...
                      users={data.subscribers
                        .filter(
                          (subscriber) =>
                            !(
                              subscriber.uid === XOApp.loginInfo.uid ||
                              subscriber.uid === XOApp.config.fileHelperUID ||
                              subscriber.uid === XOApp.config.systemUID
                            )
                        )
                        .map((item) => {
                          return new IndexTableItem(
