    ...
    render(): ReactNode {
        ...
        return <div className="xo-channelmanagerlist">
            <SmallTableEdit addTitle="添加管理员" items={data.subscribers.filter((s) => {
                ...
                return {
                    id: subscriber.uid,
                    icon: XOApp.shared.avatarUser(subscriber.uid),
                    name: subscriber.remark || subscriber.name,
                    showAction: subscriber.role !== GroupRole.owner,
                    onAction: () => {
                        XOApp.dataSource.channelDataSource.managerRemove(data.channel, [subscriber.uid]).catch((err) => {
                            Toast.error(err.msg)
                        })
                    }
                }
                ...
                routeContext.push(<UserSelect onSelect={(items) => {
                    ...
                    onFinish: async () => {
                        ...
                        await XOApp.dataSource.channelDataSource.managerAdd(data.channel, selectItems.map((item) => {
                            return item.id
                        })).catch((err) => {
                            Toast.error(err.msg)
                        })
