import { Channel, ChannelTypeGroup, ChannelTypePerson, ConversationAction, XOSDK, Mention, Message, MessageContent, Reminder, ReminderType, Reply,MessageText } from "xochat_js_sdk";
...
export class Conversation extends Component<ConversationProps> implements ConversationContext {
    ...
    fowardMessageUI(message: Message): void {
        XOApp.shared.baseContext.showConversationSelect((channels: Channel[]) => {
    ...
    showUser(uid: string) {
        ...
        XOApp.shared.baseContext.showUserInfo(uid, fromChannel, vercode)
    }

    reply(message: Message): void {
        if (message.fromUID !== XOApp.loginInfo.uid) {
            const channelInfo = XOSDK.shared().channelManager.getChannelInfo(new Channel(message.fromUID, ChannelTypePerson))
            let name = ""
            if (channelInfo) {
    ...
    componentDidMount() {
        ...
        XOApp.shared.openChannel = channel
        if (this.vm.hasDraft()) {
    ...
    dealloc() {
        ...
        XOApp.shared.openChannel = undefined
        XOSDK.shared().conversationManager.openConversation = undefined
    }
    ...
    markConversationExtra() {
        ...
        XOApp.dataSource.channelDataSource.conversationExtraUpdate({
    ...
    messageUI(message: MessageWrap, last: boolean) {
        ...
        if (message.revoke) {
            ...
            MessageCell = XOApp.messageManager.getCell(message.contentType)
    ...
    uploadReadedIfNeed() {
        ...
        if (visiableMessages && visiableMessages.length > 0) {
            ...
            for (const visiableMessage of visiableMessages) {
                if (!visiableMessage.remoteExtra.readed && visiableMessage.fromUID !== XOApp.loginInfo.uid && visiableMessage.setting.receiptEnabled) {
                    unreadMessages.push(visiableMessage.message)
                }
            }
            XOSDK.shared().receiptManager.addReceiptMessages(this.channel(), unreadMessages)
        }
    ...
    render() {
        ...
        return <Provider create={() => {
            ...
            return <>
                        ...
                        <div className="xo-conversation-footer-content">
                            ...
                            <MessageInput members={this.vm.subscribers.filter((s) => s.uid !== XOApp.loginInfo.uid)} onContext={(ctx) => {
                                ...
                                if (vm.currentReplyMessage) {
                                    ...
                                    const channelInfo = XOSDK.shared().channelManager.getChannelInfo(new Channel(vm.currentReplyMessage.fromUID, ChannelTypePerson))
                                    if (channelInfo) {
                ...
                }} menus={vm.selectMessage ? XOApp.endpoints.messageContextMenus(vm.selectMessage, this).map((menus) => {
                    return {
                ...
                <ContextMenus onContext={(ctx) => {
                    ...
                    onClick: () => {
                        ...
                        XOApp.shared.baseContext.showUserInfo(this.vm.selectUID, fromChannel, vercode)
