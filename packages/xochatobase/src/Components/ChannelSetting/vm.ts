...
export class ChannelSettingVM extends ProviderListener {
    ...
    sections(context:RouteContext<ChannelSettingRouteData>) {
        return XOApp.shared.channelSettings(context)
    }

    didMount(): void {
        XOSDK.shared().channelManager.fetchChannelInfo(this.channel)
        ...
        if(this.channel.channelType !== ChannelTypePerson) {
            ...
            if(this.subscribers && this.subscribers.length>0) {
                for (const subscriber of this.subscribers) {
                    if(subscriber.uid === XOApp.loginInfo.uid) {
    ...
    reloadChannelInfo() {
        this.channelInfo = XOSDK.shared().channelManager.getChannelInfo(this.channel)
        ...
        if(this.channelInfo && this.channel.channelType === ChannelTypePerson) {
            this.subscribers = [{
                ...
                avatar: XOApp.shared.avatarUser(this.channel.channelID),
