...
import { Channel, ChannelInfo, ChannelTypePerson, XOSDK, Subscriber } from "xochat_js_sdk";
...
export class UserInfoVM extends ProviderListener {
    ...
    didMount(): void {
        ...
        this.channelInfo = XOSDK.shared().channelManager.getChannelInfo(channel)
    ...
    reloadSubscribers() {
        if (this.fromChannel && this.fromChannel.channelType !== ChannelTypePerson) {
            const subscribers = XOSDK.shared().channelManager.getSubscribes(this.fromChannel)
            if (subscribers && subscribers.length > 0) {
                for (const subscriber of subscribers) {
                    if (subscriber.uid === this.uid) {
                        this.fromSubscriberOfUser = subscriber
                        
                    }else if(subscriber.uid === XOApp.loginInfo.uid) {
    ...
    sections(context: RouteContext<UserInfoRouteData>) {
        ...
        return XOApp.shared.userInfos(context)
    }
    ...
    // 是否是本人
   isSelf() {
       return XOApp.loginInfo.uid === this.uid
   }

    reloadChannelInfo() {
        this.channelInfo = XOSDK.shared().channelManager.getChannelInfo(new Channel(this.uid, ChannelTypePerson))
        this.notifyListener()
    }
    reloadFromChannelInfo() {
        if(this.fromChannel) {
            this.fromChannelInfo = XOSDK.shared().channelManager.getChannelInfo(this.fromChannel)
            this.notifyListener()
        }
    }
}