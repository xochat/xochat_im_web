...
import { Channel, ChannelInfo, ChannelTypePerson, Conversation, XOSDK, Message, MessageContentType, MessageStatus,MessageText } from "xochat_js_sdk"
...
export class ConversationWrap {
    ...
    public get avatar() {
        if(this.channelInfo&& this.channelInfo.logo && this.channelInfo.logo !== "") {
            return `${XOApp.dataSource.commonDataSource.getImageURL(this.channelInfo.logo)}?v=${XOApp.shared.getChannelAvatarTag()}`
        }
        return XOApp.shared.avatarChannel(this.channel)
    }
    ...
    public get category()  {
        ...
        if(channelInfo.orgData.agent_uid === XOApp.loginInfo.uid) {
            return "assignMe"
        }
...
export class MessageWrap {
   ...
   private  _reasonCode?:number // 消息错误原因代码
    ...
    public get from(): ChannelInfo | undefined {
        return XOSDK.shared().channelManager.getChannelInfo(new Channel(this.fromUID, ChannelTypePerson))
    }
    ...
    public get send(): boolean {
        return this.message.fromUID === XOApp.loginInfo.uid
    }
