...
import XOApp, { MessageDeleteListener } from "../../App";
...
export class ChatVM extends ProviderListener {
    ...
    didMount(): void {
        ...
        // 根据连接状态设置标题
        this.setConnectTitleWithConnectStatus(XOSDK.shared().connectManager.status)
        ...
        XOSDK.shared().connectManager.addConnectStatusListener(this.connectStatusListener)

        // ---------- 最近会话 ----------
        this.conversationListener = (conversation: Conversation, action: ConversationAction) => {

            const channelInfo = XOSDK.shared().channelManager.getChannelInfo(conversation.channel)
            if (!channelInfo) {
                XOSDK.shared().channelManager.fetchChannelInfo(conversation.channel)
            }
        ...
        XOSDK.shared().conversationManager.addConversationListener(this.conversationListener)
        ...
        XOSDK.shared().channelManager.addListener(this.channelListener)

        this.messageDeleteListener = (message: Message, preMessage?: Message) => {
            const conversation = XOSDK.shared().conversationManager.findConversation(message.channel)
            if (conversation) {
        ...
        XOApp.shared.addMessageDeleteListener(this.messageDeleteListener)


    }
    didUnMount(): void {
        XOSDK.shared().connectManager.removeConnectStatusListener(this.connectStatusListener)
        XOSDK.shared().conversationManager.removeConversationListener(this.conversationListener)
        XOSDK.shared().channelManager.removeListener(this.channelListener)
        XOApp.shared.removeMessageDeleteListener(this.messageDeleteListener)
    }
    ...
    setConnectTitleWithConnectStatus(connectStatus: ConnectStatus) {
        if (connectStatus === ConnectStatus.Connected) {
            this.connectTitle = XOApp.config.appName
    ...
    async requestConversationList() {
        ...
        const conversations = await XOSDK.shared().conversationManager.sync({})
